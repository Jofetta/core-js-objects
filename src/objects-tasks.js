...
function makeWord(lettersObject) {
  ...
  Object.values(lettersObject)
    ...
    .sort()
    .forEach((value) => {
      const letter = Object.keys(lettersObject).filter((key) =>
        lettersObject[key].includes(value)
      );
      result += letter;
    });
  return result;
...
function sellTickets(queue) {
  ...
  queue.forEach((value) => {
    if (value === ticketCost) {
      change.push(ticketCost);
    } else if (change.includes(value - ticketCost)) {
      const changeIndex = change.find(() => value - ticketCost);
